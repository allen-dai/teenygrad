...
fn main() -> Result<(), String> {
    ...
    let mut optim =
        teenygrad::nn::optim::adam(&[&mut model.l1, &mut model.l2, &mut model.l3], 0.001);
    let batch_size = 1;
    let (mut img_batched, _, _, _) = fetch_mnist::<Cpu>(batch_size, false);
    ...
    'mainloop: loop {
        if train {
            ...
            let x: Tensor<_> = Tensor::from_shape(&*input, [28 * 28, 2]);
            let x = x.detach();
            let y: Tensor<Cpu> = Tensor::from_shape(&*y, [28 * 28, 1]);
            let y = y.detach();
            // let y: Tensor<Cpu> =
            ...
            let out_2 = &y - &out;
            let mut loss: Tensor<_> = (&out_2 * &out_2).sum_all() / (28 * 28);
            loss.backward();
            optim.step();
            for r in 0..28 {
                for c in 0..28 {
            ...
            let out = model.forward(&Tensor::from_shape(&*input, [28 * 28 * 4, 2]));
            let out_vec = out.to_vec();
            for r in 0..28 * 2 {
                for c in 0..28 * 2 {
            ...
            let out = model.forward(&Tensor::from_shape(
                &*input,
