...
impl CstyleLanguage {
    ...
    pub fn render_kernel(
        ...
        for (i, (name, dtype)) in bufs.iter().enumerate() {
            let s = if dtype.name.starts_with("image") {
                format!(
                    "{} image2d_t",
        ...
        match uop {
            ...
            UOps::ALU => {
                        ...
                        || args[0] == OpType::Binary(Binary::Mul))
                {
                    ...
                    let b = r[&vin[1]].clone();
                    val = match &args[0] {
                        Args::Op(op) => lang.call(&op, vec![a, b], None),
                        _ => unreachable!(),
                    }
                } else {
                    let a = r[&vin[0]].clone();
                    ...
                    val = match &args[0] {
                        Args::Op(op) => lang.call(&op, vec![a, b, c], None),
                        _ => unreachable!(),
                    }
                }
